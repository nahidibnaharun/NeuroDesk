import React, { useState } from 'react';
import { explainCode } from '../services/geminiService';
import { simpleMarkdownToHtml } from '../utils/formatters';
import { CodeExplanationItem } from '../types';
import { CodeIcon, SparklesIcon } from './Icon';

interface CodeExplainerProps {
  onSave: (item: CodeExplanationItem) => void;
}

const CodeExplainer: React.FC<CodeExplainerProps> = ({ onSave }) => {
    const [code, setCode] = useState('');
    const [explanation, setExplanation] = useState('');
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState('');
    const [isSaved, setIsSaved] = useState(false);

    const handleExplainCode = async () => {
        if (!code.trim()) {
            setError('Please paste some code to explain.');
            return;
        }
        setError('');
        setIsLoading(true);
        setExplanation('');
        setIsSaved(false);
        try {
            const result = await explainCode(code);
            setExplanation(result);
        } catch (e: any) {
            setError(e instanceof Error ? e.message : 'An unknown error occurred.');
        } finally {
            setIsLoading(false);
        }
    };
    
    const handleSave = () => {
        if (explanation) {
            const newItem: CodeExplanationItem = {
                id: new Date().toISOString(),
                type: 'codeExplanation',
                content: explanation,
                sourceCode: code,
                timestamp: new Date().toLocaleString(),
            };
            onSave(newItem);
            setIsSaved(true);
        }
    };


    return (
        <div className="p-4 sm:p-6 md:p-8 h-full flex flex-col">
            <div className="bg-white/80 backdrop-blur-sm border border-slate-200 rounded-xl shadow-lg p-6 sm:p-8 h-full flex flex-col">
                <div className="flex items-center mb-4">
                    <CodeIcon className="w-6 h-6 mr-3 text-indigo-500" />
                    <h2 className="text-xl font-bold text-slate-800">AI Code Explainer</h2>
                </div>
                <p className="text-slate-500 mb-6">Paste any code snippet and get a detailed, step-by-step explanation of how it works.</p>
                
                <div className="flex flex-col gap-4 mb-4">
                    <textarea
                        value={code}
                        onChange={(e) => setCode(e.target.value)}
                        placeholder="Paste your code here..."
                        className="w-full h-48 p-3 bg-slate-900 text-slate-100 font-mono text-sm rounded-lg border border-slate-700 resize-y focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                        disabled={isLoading}
                    />
                    <button
                        onClick={handleExplainCode}
                        disabled={isLoading || !code}
                        className="w-full sm:w-auto self-end flex justify-center items-center px-6 py-3 animated-gradient-bg text-white font-bold rounded-lg shadow-md disabled:bg-slate-400 disabled:cursor-not-allowed transition-all disabled:animate-none"
                    >
                        <SparklesIcon className="w-5 h-5 mr-2" />
                        {isLoading ? 'Explaining...' : 'Explain Code'}
                    </button>
                </div>

                {error && <div className="text-red-500 bg-red-100 p-3 rounded-lg mb-4">{error}</div>}

                <div className="flex-grow bg-slate-50 rounded-lg p-4 border border-slate-200 overflow-y-auto min-h-[200px]">
                    {isLoading && (
                        <div className="flex flex-col justify-center items-center h-full">
                            <div className="flex items-center justify-center space-x-2">
                                <div className="w-3 h-3 bg-indigo-500 rounded-full dot-1"></div>
                                <div className="w-3 h-3 bg-indigo-500 rounded-full dot-2"></div>
                                <div className="w-3 h-3 bg-indigo-500 rounded-full dot-3"></div>
                            </div>
                            <p className="mt-4 text-slate-500">AI is analyzing your code...</p>
                        </div>
                    )}
                    {explanation ? (
                        <div className="prose prose-sm max-w-none text-slate-700 animate-fade-in-slide-up" dangerouslySetInnerHTML={{ __html: simpleMarkdownToHtml(explanation) }} />
                    ) : (
                        !isLoading && <p className="text-slate-400 text-center mt-8">Your code explanation will appear here.</p>
                    )}
                </div>
                {explanation && (
                    <button
                        onClick={handleSave}
                        disabled={isSaved}
                        className="mt-6 w-full px-6 py-3 animated-gradient-bg-green text-white font-bold rounded-lg shadow-md disabled:bg-green-400 disabled:cursor-not-allowed transition-all disabled:animate-none"
                    >
                        {isSaved ? 'Saved to History' : 'Save Explanation to History'}
                    </button>
                )}
            </div>
        </div>
    );
};

export default CodeExplainer;